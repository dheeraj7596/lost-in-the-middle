...
def main(
    ...
    input_data = load_file(input_path)
    for id, input_example in enumerate(input_data):
        question = input_example["question"]
        documents = []
        for ctx in deepcopy(input_example["ctxs"][:top_n]):
            documents.append(Document.from_dict(ctx))
        if not documents:
            raise ValueError(f"Did not find any documents for example: {input_example}")

        prompt = get_qa_prompt_with_task(
            question,
            documents,
            task,
        )
    ...
    if debug:
        ...
        all_model_documents = all_model_documents[:100]
    ...
    for p, s in zip(prompts, raw_responses):
        ...
        ans = s.replace(model.tokenizer.eos_token, "").replace("<s>", "").strip().split(p.replace("<s>", "").strip())[
            -1].strip()
        ans = postprocess_answers_closed(ans, task, choices)
    ...
    for output, item in zip(responses, examples):
        if metric == "em":
            metric_result = metric_max_over_ground_truths(
                exact_match_score, output, item["golds"])
        elif metric == "accuracy":
            metric_result = 1.0 if item["golds"][0] in output else 0.0
        elif metric == "match":
            metric_result = match(output, item["golds"])
        else:
            raise NotImplementedError
        item["metric_result"] = metric_result

    print("overall result: {0}".format(
        np.mean([item["metric_result"] for item in examples])))
